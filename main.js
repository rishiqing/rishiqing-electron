...
if (process.platform !== "darwin")
  app.setAppUserModelId("release.rishiqing.electron"); // 在这里设置appId，win10才能正常推送通知
...
class Main {
  ...
  async _createWindow() {
    ...
    pkg.env === "dev"
      ? this.mainWindow.loadURL(`http://localhost:8080/index`)
      : this.mainWindow.loadURL(`file://${__dirname}/dist/index/index.html`);
    // 打开调试窗口
    ...
    mainDb.event.on(mainDb.EVENTS.DevTool, () => {
      webContents.openDevTools();
    });
  }
  ...
  _onWindowClose(e) {
    if (!global.force_close) {
      // 这个force_close是在native/tray.js里设置的，当时点击托盘里的退出按钮时，被置为true
      if (process.platform === "darwin") {
